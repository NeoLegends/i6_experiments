...
def dump(dataset, options):
  ...
  output_dict["target_classes"] = rnn.engine.network.get_extern_data("classes").get_placeholder_as_batch_major()
  ...
  while dataset.is_less_than_num_seqs(seq_idx) and seq_idx <= options.endseq:
    ...
    fig, axs = plt.subplots(2, weights.shape[3], figsize=(weights.shape[3] * 30, 35))
    ...
    for head in range(weights.shape[3]):
      ...
      if rnn.config.typed_value("att_area") == "win" and rnn.config.typed_value("att_win_size") == "full":
        ...
        # tick_labels_x = ["---"] * excess_size + list(target_classes) + ["---"] * excess_size
      else:
        ...
        # tick_labels_x = list(target_classes)
      ...
      target_align_borders_expanded_center = [
        (i + j) / 2 for i, j in zip(target_align_borders_expanded[:-1], target_align_borders_expanded[1:])]
      target_align_borders = [i + 1 for i, x in enumerate(target_align) if x != targetb_blank_idx]
      if target_align_borders[0] != 0:
        target_align_borders = [0] + target_align_borders
      target_align_borders_center = [
        (i + j) / 2 for i, j in zip(target_align_borders[:-1], target_align_borders[1:])]
      target_align_major = [x for x in target_align if x != targetb_blank_idx]

      # print(hmm_align)
      # print(hmm_align_borders)
      # print(hmm_align_borders_center)
      # print([(i, x) for i, x in enumerate(hmm_align)])
      # print([(i, x) for i, x in enumerate(hmm_align_borders)])
      # print(hmm_align_major)
      ...
      for ax, matrix, title in zip(axs[:, head] if weights.shape[3] > 1 else axs, [weights_, energies_], ["weights", "energies"]):
        ...
        ax.set_title(title + "_head" + str(head), y=1.15)

        ax.set_yticks([x - .5 for x in target_align_borders])
        ax.yaxis.set_major_formatter(ticker.NullFormatter())
        ax.set_yticks([x - .5 for x in target_align_borders_center], minor=True)
        ...
        # ax.set_xticklabels(target_align_major, fontsize=9, minor=True, rotation=45)
        ax.tick_params(axis="x", which="minor", length=0)
        ax.tick_params(axis="x", which="major", length=10)
        ax.set_xlim(-.5, weights_.shape[1] - .5)
        ax.set_ylim(weights_.shape[0] - .5, -.5)
        ax.set_xlabel("RNA BPE Alignment (scaled to unpooled time)")
        ax.set_ylabel("RNA BPE Alignment")
        ax.xaxis.tick_top()
        ...
        for idx in target_align_borders:
          ax.axhline(y=idx - .5, xmin=0, xmax=1, color="k", linewidth=.5)

        plt.colorbar(matshow, ax=ax)

    fig.suptitle(seq_tag[0], y=0.98)
    # fig.tight_layout()
    plt.savefig(os.path.join(subfolder, options.plot_name + "_seq-" + str(seq_idx) + ".png"))
    plt.close()
...
def main(argv):
  ...
  arg_parser.add_argument('--plot_name', type=str)
