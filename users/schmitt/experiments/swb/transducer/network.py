...
def get_extended_net_dict(pretrain_idx):
  ...
  net_dict.update({
    ...
    "encoder": {
      "class": "linear", "from": "encoder0", "n_out": EncValueTotalDim // EncValueDecFactor, "activation": None} if EncValueDecFactor != 1 else {
  ...
  if slow_rnn_extra_loss:
    ...
    net_dict.update({
      "slow_rnn": {
        "class": "masked_computation", "mask": "output/output_emit",
        "from": "output/lm", "unit": {"class": "copy", "from": ["output/s", "data", "output/att", ]}},
      "slow_readout_in": {
